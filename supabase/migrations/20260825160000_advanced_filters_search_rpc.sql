...
BEGIN
  ...
  WHERE v.status = 'active'
    ...
    AND (p_rental_period IS NULL OR v.price_period ILIKE p_rental_period OR v.module_data->>'rental_period' ILIKE p_rental_period)
    AND (p_job_type IS NULL OR v.job_type ILIKE p_job_type OR v.module_data->>'job_type' ILIKE p_job_type)
    AND (p_work_mode IS NULL OR v.work_mode ILIKE p_work_mode OR v.module_data->>'work_mode' ILIKE p_work_mode)
    AND (p_pricing_type IS NULL OR v.pricing_type ILIKE p_pricing_type OR v.module_data->>'pricing_type' ILIKE p_pricing_type)
    AND (p_emergency_service IS NULL OR (v.module_data->>'emergency_service')::boolean = p_emergency_service)
    AND (p_delivery_available IS NULL OR (v.module_data->>'delivery_available')::boolean = p_delivery_available)
    AND (
