...
export const AdminReportsView: React.FC<Props> = ({ staff, onRefresh, onNavigateToTarget }) => {
  const [rows, setRows] = useState<AdminReport[]>([]), [counts, setCounts] = useState<Record<string, number>>({}), [total, setTotal] = useState(0);
  const [selected, setSelected] = useState<AdminReport | null>(null), [loading, setLoading] = useState(true), [busy, setBusy] = useState(false);
  ...
  return <section className="space-y-5 pb-24">
    ...
    <div className="grid gap-2 rounded-2xl border bg-white p-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <select value={status} onChange={event => { setStatus(event.target.value); setPage(1); }} className="rounded-xl border p-2.5 text-sm"><option value="all">All statuses</option><option value="submitted">Submitted</option><option value="under_review">Under review</option><option value="resolved">Resolved</option><option value="dismissed">Dismissed</option></select>
      <select value={target} onChange={event => { setTarget(event.target.value); setPage(1); }} className="rounded-xl border p-2.5 text-sm"><option value="all">All targets</option><option value="listing">Listings</option><option value="review">Reviews</option><option value="user">Users</option><option value="message">Messages</option></select>
      <select value={module} onChange={event => { setModule(event.target.value); setPage(1); }} className="rounded-xl border p-2.5 text-sm"><option value="all">All modules</option><option value="rental">Rentals</option><option value="job">Jobs</option><option value="service">Services</option></select>
      <input value={reason} onChange={event => { setReason(event.target.value || 'all'); setPage(1); }} placeholder="Reason code or all" className="rounded-xl border p-2.5 text-sm" />
      <select value={assignee} onChange={event => { setAssignee(event.target.value); setPage(1); }} className="rounded-xl border p-2.5 text-sm"><option value="all">Any assignee</option><option value="mine">Assigned to me</option></select>
