...
export const BRAND_ASSETS = { master: '/brand/rentoura-logo-master.png', mark: '/brand/rentoura-logo-mark.png', social: '/brand/og-default.png' } as const;
...
export const RentouraLogo: React.FC<RentouraLogoProps> = ({ variant = 'header', theme = 'dark-header', className = '', size = 'md', showTagline = false }) => {
  ...
  return <span className={`inline-flex ${stacked ? 'flex-col' : 'flex-row'} items-center justify-center gap-2 select-none ${className}`}>
    <img src={BRAND_ASSETS.mark} alt="RENTOURA.LK logo" width="581" height="572" className={`${dimensions[size]} w-auto shrink-0 object-contain`} decoding="async" draggable={false} />
