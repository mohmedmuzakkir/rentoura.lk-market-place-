...
export interface ModerationQueueFilters { module?:string;categoryId?:string;provinceId?:string;submittedFrom?:string;submittedTo?:string;reported?:boolean;search?:string;cursorAt?:string;cursorId?:string;limit?:number }
export interface AdminQueueListing { id:string;ownerId:string;ownerName:string;module:string;categoryId:string|null;categoryName:string;provinceId:string|null;provinceName:string;title:string;summary:string;description:string;price:number|null;pricingPeriod:string;status:string;submittedAt:string;createdAt:string;updatedAt:string;reported:boolean;coverPath:string|null }
export interface ModerationQueuePage { rows:AdminQueueListing[];hasMore:boolean;pageSize:number }
export interface ModerationAuditEvent { id:string;actorId:string|null;actorName:string;actorRole:string;action:string;listingId:string;listingTitle:string;reason:string|null;module:string|null;createdAt:string }
export interface StaffModerationMetric { actorId:string;actorName:string;actorRole:string;eventDate:string;reviewed:number;approved:number;rejected:number;changesRequested:number }
export interface AdminDashboardMetrics { totalListings:number;activeListings:number;pendingListings:number;rejectedListings:number;reportedListings:number;totalUsers:number;rentalsCount:number;jobsCount:number;servicesCount:number;newListings7d:number;previousListings7d:number;newUsers7d:number;previousUsers7d:number }
...
export class AdminModerationService {
  static async getPendingQueue(filters:ModerationQueueFilters={}):Promise<ModerationQueuePage>{
    ...
    return{pageSize,hasMore:Boolean(result.has_more),rows:raw.slice(0,pageSize).map(row=>({id:String(row.id),ownerId:String(row.owner_id),ownerName:String(row.owner_name||'Owner'),module:String(row.module),categoryId:row.category_id?String(row.category_id):null,categoryName:String(row.category_name||'Uncategorized'),provinceId:row.province_id?String(row.province_id):null,provinceName:String(row.province_name||'Sri Lanka'),title:String(row.title),summary:String(row.short_summary||''),description:String(row.description||''),price:row.price===null?null:Number(row.price),pricingPeriod:String(row.pricing_period||''),status:String(row.status),submittedAt:String(row.submitted_at||row.created_at),createdAt:String(row.created_at),updatedAt:String(row.updated_at),reported:Boolean(row.reported),coverPath:row.cover_path?String(row.cover_path):null}))};
  }
